...
int Handler::run_parallel(Param* PM) {
    ...
    auto    begin = std::chrono::steady_clock::now();
    {
        solver->setInputParam(PM);
        solver->setInputDataSet(&DS);
        solver->initializeKernel(0);  // required!!!
        ...
        for (int icycle = istart, icalc = 0; icycle < iend; ++icycle, ++icalc) {
            ...
            solver->initializeKernel(icycle);
            solver->executeKernel(icycle);
...
int Handler::run_parallel_22(Param* PM) {
    DataSet DS;
    auto    begin = std::chrono::steady_clock::now();
    {
        solver->setInputParam(PM);
