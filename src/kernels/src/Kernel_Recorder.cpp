...
void Kernel_Recorder::setInputParam_impl(std::shared_ptr<Param> PM)
{
    ...
    time_unit = _param->get_real({"model.time_unit", "solver.time_unit"}, LOC(),
                                 phys::time_d, 1.0f);
...
void Kernel_Recorder::parse()
{
    ...
    while (true)
    {
        ...
        else if (_param->is_object(firstkey))
        {
            ...
            if (_param->has_key(mode_key))
                mode = _param->get_string({mode_key}, LOC());
            if (_param->has_key(save_key))
                save = _param->get_string({save_key}, LOC());
        ...
        else if (_param->is_array(firstkey))
        {
            ...
            std::string v0, v1;
            if (_param->has_key(firstv0key))
                v0 = _param->get_string({firstv0key}, LOC());
            if (_param->has_key(firstv1key))
                v1 = _param->get_string({firstv1key}, LOC());
        ...
        if (std::find(opened_files.begin(), opened_files.end(), save) ==
                opened_files.end() &&
            mode == "average")
        {
            std::shared_ptr<RuleEvaluator> record_time_rule( //
                new RuleEvaluator(
                    "time(t{flowcontrol}:R, pertimeunit{flowcontrol}:R)",
                    _dataset, mode, save, nsamp_ptr[0]));
            _ruleset->registerRules(record_time_rule);
            opened_files.push_back(save);
        }

        if (occ0 >= 0)
        { // replace some env in rules
...
{
    for (auto& irule : _ruleset->getRules())
    {
        irule->calculateResult(isamp_ptr[0]);
    }
