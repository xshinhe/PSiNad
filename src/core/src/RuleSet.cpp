    ...
    for (auto it = expression_ios.begin(); it != expression_ios.end(); ++it) {
        if (exprRule->save == (*it)->unique_name) {
            if (exprRule->mode != (*it)->rules[0]->mode) {
                ...
                throw psnd_error(utils::concat("All rules in one RuleSet must have the same! Here ", exprRule->rule,
                                               " meets a confliction"));
            }
...
void RuleSet::flush(const std::string& path, const std::string& suff, int level) {
    ...
    if (level < 0) {
        for (auto& r : rules) {
            if (r->mode == RuleEvaluatorPolicy::stat) { r->writeTo(ofs, r->result->dataPointerTrace, 0); }
        }
        ofs << "\n";
    } else {
...
void RuleSet::appendHeader(std::shared_ptr<RuleEvaluator>& expr_rule) {
    ...
    if (res->vtype != VariableDescriptorPolicy::TabularOutput && res->vtype != VariableDescriptorPolicy::MetaOutput)
        return;
...
Result RuleSet::getResult() {
    Result res{};
    for (auto& r : rules) {
        if (r->result->vtype != VariableDescriptorPolicy::TabularOutput) continue;
