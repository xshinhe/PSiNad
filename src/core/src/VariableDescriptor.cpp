 ...
 */
VariableDescriptor::VariableDescriptor(const std::string& token_string, const std::string& save,
    ...
    : tokenString{token_string},  // The original token string
      ...
      vtype{vtype_in} {           // Flag indicating if the variable is a tabular output
    ...
    if (std::regex_match(tokenString, match, pattern)) {
        ...
        std::tie(name, field, time, index, type) = std::make_tuple(match[1].str(),  // Name of the variable
    ...
    if (vtype == VariableDescriptorPolicy::TabularOutput && !name.empty() && name[0] == '*') {
    ...
    keyRaw  = utils::concat(field, ".", name);  // Raw key is a concatenation of field and name
    ...
    } else                              // @TODO: so {@} cannot be used for TabularOutput
    {
        ...
        if (time.empty()) {
            ...
            // only by reference
            keyRec = keyRaw;  // No time specified, record key is the same as raw key
        } else {
...
void VariableDescriptor::defineIn(std::shared_ptr<DataSet> DS, kids_dtype data_type,
    ...
    switch (vtype) {
        ...
        case VariableDescriptorPolicy::MetaOutput: {
            ...
            cxxstackedshape.push_back(totalFrameNumber);  // Prepend the total number of frames
            for (const auto& dim : cxxshape) {
                cxxstackedshape.push_back(dim);  // Append the original dimensions
            }
