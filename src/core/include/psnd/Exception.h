...
struct psnd_error : public std::runtime_error {
    psnd_error(std::string const text) : std::runtime_error(text) {}
};

struct param_warning : public psnd_error {
    param_warning(std::string const text) : psnd_error(utils::concat("parameter warning of : ", text)) {}
};

#define psnd_assert(condition, info) \
    if (!(condition)) throw psnd_error(utils::concat(info, "\nassert on: [", #condition, "] fails!"));
