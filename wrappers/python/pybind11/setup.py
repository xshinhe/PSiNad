"""
setup.py: Used for building python wrappers for pykids
"""
import ast
import re
import os
...
def removePackage(mod, verbose):
        try:
            ...
            return
        if len(pathList) > 1:
           raise Exception("more than one item in pykids.__path__")
...
def uninstall(verbose=True):
    ...
    try:
        import pykids.app as app
        removePackage(app, verbose)
    except ImportError:
        pass
...
def writeVersionPy(filename="pykids/version.py", major_version_num=MAJOR_VERSION_NUM,
...
def buildKeywordDictionary(major_version_num=MAJOR_VERSION_NUM,
    ...
    extensionArgs = {"name": "pykids._pykids",
                    "sources": ["src/swig_doxygen/KIDSSwig.cxx"],
    ...
    ## OTHER EXTENSION                                    
    setupKeywords["ext_modules"] += cythonize('pykids/ext/*.pyx')
    setupKeywords["ext_modules"] += cythonize(Extension(
        "pykids.ext.examples",
        sources=[
            "pykids/ext/examples/src/test.cpp",
            "pykids/ext/examples/test.pyx",
        ],
        include_dirs=include_dirs +[
            "pykids/ext/examples/include",
            "pykids/ext/examples/",
            numpy.get_include(),
        ],
        language="c++",
