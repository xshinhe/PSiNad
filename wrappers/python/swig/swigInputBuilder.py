...
def stripNSPrefix(name, rePattern=NS_RE_PATTERN):
    try:
...
def getClassMethodList(classNode, skipMethods):
    ...
    shortClassName=stripNSPrefix(className)
    methodList=[]
    for section in findNodes(classNode, "sectiondef", kind="public-static-func")+findNodes(classNode, "sectiondef", kind="public-func"):
        for memberNode in findNodes(section, "memberdef", kind="function", prot="public"):
            ...
            shortMethDefinition=stripNSPrefix(methDefinition)
...
class SwigInputBuilder:
    ...
    def writeFactories(self):
        ...
        for classNode in findNodes(self.doc.getroot(), "compounddef", kind="class", prot="public"):
            ...
            for baseNodePnt in findNodes(classNode, "basecompoundref", prot="public"):
                if "refid" in baseNodePnt.attrib:
                    ...
                    if baseName == 'OpenMM::Model':
                        modelSubclassList.append(shortClassName)
                    elif baseName in ('OpenMM::Integrator'):
        ...
        self.fOut.write("%factory(OpenMM::Model& OpenMM::System::getForce")
        for name in sorted(modelSubclassList):
            self.fOut.write(",\n         OpenMM::%s" % name)
        self.fOut.write(");\n\n")

        self.fOut.write("%factory(OpenMM::Model* OpenMM_XmlSerializer__cloneForce")
        for name in sorted(modelSubclassList):
            self.fOut.write(",\n         OpenMM::%s" % name)
        self.fOut.write(");\n\n")

        self.fOut.write("%factory(OpenMM::Model* OpenMM_XmlSerializer__deserializeForce")
        for name in sorted(modelSubclassList):
            self.fOut.write(",\n         OpenMM::%s" % name)
        self.fOut.write(");\n\n")

        self.fOut.write("%factory(OpenMM::Model& OpenMM::CustomCVForce::getCollectiveVariable")
        for name in sorted(modelSubclassList):
            self.fOut.write(",\n         OpenMM::%s" % name)
        self.fOut.write(");\n\n")
    ...
    def writeGlobalConstants(self):
        ...
        for section in findNodes(node, "sectiondef", kind="var"):
            for memberNode in findNodes(section, "memberdef", kind="variable", mutable="no", prot="public", static="yes"):
                vDef = stripNSPrefix(getText("definition", memberNode))
    ...
    def writeForwardDeclarations(self):
        ...
        for classNode in self._orderedClassNodes:
            ...
            className = stripNSPrefix(getText("compoundname", classNode))
    ...
    def writeClassDeclarations(self):
        ...
        for classNode in self._orderedClassNodes:
            ...
            for baseNodePnt in findNodes(classNode, "basecompoundref", prot="public"):
                if "refid" in baseNodePnt.attrib:
                    baseName = stripNSPrefix(getText(".", baseNodePnt))
