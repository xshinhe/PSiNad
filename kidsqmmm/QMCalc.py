...
from sharcQMDriver import SharcInterfaceInput, SharcInterfaceOutput  # objects to handle QM with SHARC QM Interfaces
...
import CBF
import qmmmenv  # functions to handle third-party software environment
...
class QM:
    ...
    def __init__(self, cobcom, command, geometry, charges, step, restart=None, displacement=None,
        ...
        if command[51] == '1':  # QM calculation by Gaussian
        ...
        elif command[51] == '10086':  # QM calculation by MNDO

            # extract sections of the command file that are related to the QM code
            keys = CBF.ReadCobramCommand(cobcom, 'mndo', '')  # gaussian keywords
            mndoadd = CBF.ReadCobramCommand(cobcom, 'mndoadd', '')  # gaussian additional parts
            ...
            optdict["EMB"] = True
            # background charges, only for a calculation with atoms in the H or M layers
            if geometry.NatomMM > 0 and optdict['EMB']: 
        ...
        elif command[51] == '10087':  # QM calculation by BAGEL

            # extract sections of the command file that are related to the QM code
            keys = CBF.ReadCobramCommand(cobcom, 'bagel', '')  # gaussian keywords
            bageladd = CBF.ReadCobramCommand(cobcom, 'bageladd', '')  # gaussian additional parts
            ...
            self.inputData = BagelInput(keys, bageladd, modelHgeom,
                                           geometry.getAtomLabels("modelH"), optdict)
    ...
    # =============================================================================================================
    @staticmethod
    def _lauchMNDOQM(rundir, filename, qmexe):
        """ This simple function executes the command for running MNDO QM
         It is needed to store the functions to execute for a parallel execution """
        ...
        # environmental variables for MOLCAS execution
        os.putenv('Project', 'MNDO')
        os.putenv('WorkDir', os.getcwd())
        os.putenv('MNDO_OUTPUT', os.getcwd())

        # run qm calculation
        with open(filename + ".inp", "r") as finp:
            with open(filename + ".log", "w") as fout:
                with open(filename + ".err", "w") as ferr:
                    subprocess.call(["nice", "-1", qmexe], stdin=finp, stdout=fout, stderr=ferr)
    ...
    # =============================================================================================================
    @staticmethod
    def _lauchBagelQM(rundir, filename, qmexe):
        """ This simple function executes the command for running MNDO QM
         It is needed to store the functions to execute for a parallel execution """
        ...
        # environmental variables for MOLCAS execution
        os.putenv('Project', 'BAGEL')
        os.putenv('WorkDir', os.getcwd())
        os.putenv('BAGEL_OUTPUT', os.getcwd())
        os.putenv('BAGEL_NUM_THREAD', 1)

        # run qm calculation
        with open(filename + ".json", "r") as finp:
            with open(filename + ".log", "w") as fout:
                with open(filename + ".err", "w") as ferr:
                    subprocess.call(["nice", "-1", qmexe], stdin=finp, stdout=fout, stderr=ferr)
    ...
    def runQM(QMcalculations, memory="500MB", nprocQM=1, nprocParall=1):
        ...
        for qm in QMcalculations:
            ...
            if isinstance(qm.inputData, GaussianInput):
                ...
                checkResults = qmmmenv.checkGaussianQMEnv()
            ...
            # input should be run with gaussian!
                ...
                # check if there is a working environment for gaussian
                checkResults = qmmmenv.checkMNDOQMEnv()
                if not checkResults[0]:
                    # if not, stop execution with error message
                    logwrt.fatalerror(checkResults[1])

                # name of the executable and of the I/O files
                qmexe, filename = os.environ["MNDO_EXE_QM"], "mndo-QM"
                # set the name where the qm calculation will be run, store it, and move there
                rundir = QM._availablePathForQM()
                # store the names of the dir and files for the current calculation
                rundirList.append(rundir), filenameList.append(filename)
                ...
                # write the input text to file
                with open(os.path.join(rundir, filename + ".inp"), "w") as finp:
                    finp.write(qm.inputData.fileText(filename, memory=memory, nproc=nprocQM, gversion=qmexe))

                # copy restart file to the working directory, to use the file for restart (by molden.dat)
                if qm.inputData.otheropt["restart"] is not None:
                    shutil.copy(qm.inputData.otheropt["restart"], os.path.join(rundir, filename + ".dat"))

                # when parallel calculation, use apply_async method to run asincronously
                if pool is not None:
                    pool.apply_async(QM._lauchMNDOQM, args=(rundir, filename, qmexe))
                else:  # when serial calculation, run normally
                    QM._lauchMNDOQM(rundir, filename, qmexe)

            # input should be run with gaussian!
            elif isinstance(qm.inputData, BagelInput):

                # check if there is a working environment for gaussian
                checkResults = qmmmenv.checkBagelQMEnv()
                if not checkResults[0]:
                    # if not, stop execution with error message
                    logwrt.fatalerror(checkResults[1])

                # name of the executable and of the I/O files
                qmexe, filename = os.environ["BAGEL_EXE_QM"], "bagel-QM"
                # set the name where the qm calculation will be run, store it, and move there
                rundir = QM._availablePathForQM()
                # store the names of the dir and files for the current calculation
                rundirList.append(rundir), filenameList.append(filename)
                ...
                # when parallel calculation, use apply_async method to run asincronously
                if pool is not None:
                    pool.apply_async(QM._lauchBagelQM, args=(rundir, filename, qmexe))
                else:  # when serial calculation, run normally
                    QM._lauchBagelQM(rundir, filename, qmexe)
            ...
            # input is of type SHARC INTERFACE
                ...
                checkResults = qmmmenv.checkMolcasEnv()
                if not checkResults[0]:
                    # if not, stop execution with error message
                    logwrt.fatalerror(checkResults[1])
        ...
        for qm, filename, rundir in zip(QMcalculations, filenameList, rundirList):
            ...
            if isinstance(qm.inputData, GaussianInput):
                ...
                #input is of type MOLCAS INTRFACE
                qm.outputData = MNDOOutput(filename, rundir, qm.inputData.calctype)

            elif isinstance(qm.inputData, BagelInput):
                #input is of type MOLCAS INTRFACE
                qm.outputData = BagelOutput(filename, rundir, qm.inputData.calctype, qm.inputData.otheropt["SP"])
