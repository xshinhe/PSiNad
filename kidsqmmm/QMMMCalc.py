...
class QMMM:
    ...
    def __init__(self, command, geometry, QM_Results, MM_Results, step, prevx1=None, prevx2=None):
        ...
        if "H" not in geometry.calculationType:  # MM only calculation: extract MM results
            ...
            for nstate in QM_Results.gradientdict:
                ...
                if QM_Results.gradient(nstate) is None or \
                    ...
                    gradQMMM = subtractiveScheme(command, geometry, QM_Results.gradient(nstate), gradMM_modelH,
            ...
            for istate in QM_Results.nacdict:
                ...
                for kstate in QM_Results.nacdict[istate]:
                    ...
                    if QM_Results.nacCRGdict is not None:
                        ...
                        nacQMMM = ReduceNacScheme(command, geometry, nacQM, nacCRG, correctAtomLink=doCorrect)
                    else:
                        nacQMMM = nacQM
            ...
            if command[19] == '0':
                ...
                for i in range(geometry.NatomHM):
                    if geometry.list_MEDIUM_HIGH[i] in geometry.list_HIGH:
                        ...
                        logwrt.fatalerror('Atom ' + str(geometry.list_MEDIUM_HIGH[i]) + ' not found among HL and ML atoms!')
                nac = np.array(nac)
...
def ReduceNacScheme(command, geometry, nacQM, nacCRG, correctAtomLink=True):
    x, y, z = [], [], []
    ...
    for i in range(geometry.atomNum):
        ...
        if i+1 in geometry.list_MEDIUM:
            ...
            # when requested, correct the gradient of the link atoms by distributing the H atom gradient
            if command[130] == '0' and correctAtomLink:
                # select the atom links in which the a atom is the i+1 HIGH atom
                for ilink, b, a in zip(range(len(geometry.atomLink_BA[0])), *geometry.atomLink_BA):
                    if i+1 == b:
                        # compute the bond length ration that is necessary to evaluate dR(L)/d(R(Q))
...
def subtractiveScheme(command, geometry, gradQM_modelH, gradMM_modelH, gradMM_real_nocharge, gradQM_coulomb,
...
def GradientProjectionOnNormalModes(geometry, command, gradient):
    """Compute the projection of the gradient on the set of normal modes below a frequency threshold defined by command[41] """
    
    freqdir = os.getcwd() 
    # parse the results of the frequency calculation run using CobrammOutput
    freqresults = CobrammOutput(freqdir, store_files=True)
    ...
    for i in range(len(mol_oscillator.eigenVal)):
        if mol_oscillator.eigenVal[i] > 0. and mol_oscillator.eigenVal[i] < ( float(command[40]) * constants.wavnr2au ) ** 2 and i == 4:
