...
class Charge:
    ...
    def __init__(self, geometry, CRG_real, CRG_model_H):
        ...
        if TotCRG_modelH_tmp != round(TotCRG_modelH_tmp):
            logwrt.writelog("Total from model-H top is not an integer!\n")
            if (TotCRG_modelH_tmp-round(TotCRG_modelH_tmp)) < 0.005:
                logwrt.writelog("Try to rescale charges from modelH from {} to {}!\n".format(
                ...
                logwrt.fatalerror("cannot rescale the model-H charges\n")
    ...
    def embeddingCharges(self, geometry, CRG_real, CRG_model_H): # CRG_model_H is intrinsically from model-H.top
        ...
        if debug:
            logwrt.writelog('Using \'embeddingCharges\' to create the charge embedding used in QM calculation\n')
            if geometry.NsubH == 0:
                logwrt.writelog('There are no atom-links, so the embedding charges will not be modified\n')
            else:
                logwrt.writelog('There are {0} atom-links\n'.format(geometry.NsubH))
                logwrt.writelog('The embedding scheme requested by is: {0}\n'.format(whichemb))
                if 0 <= whichemb < 10:
                    logwrt.writelog('Embedding charges will not be modified\n')
                    logwrt.writelog('Use this option with care: it will result in the loss of charge!\n')
                elif 10 <= whichemb < 20:
                    logwrt.writelog('Charges of MM frontier atoms will be zeroed\n')
                    logwrt.writelog('Use this option with care: it will result in the loss of charge!\n')
                elif 20 <= whichemb < 30:
                    logwrt.writelog('Charges of MM frontier atoms will be distributed among the closest MM atoms\n')

        # Print debug info on model comparison
        if debug:
            logwrt.writelog('\n' + '=' * 80 + '\n')
            logwrt.writelog('Comparison between charges of "model" (as a part of "real")    \n')
            logwrt.writelog('                          and "model" (as a part of "model-H") \n')
            ...
            logwrt.writelog('=' * 80 + '\n\n')
            if whichemb == 25:
                logwrt.writelog('Delta_tot will not be distributed\n\n')
            else:
                logwrt.writelog('Delta_tot will be equally distributed among the ' + str(geometry.NsubH)
                                + ' atom-links\n\n')
        ...
        # redistribute charges
            ...
            for iAtomM1, atomM2Charge in zip(geometry.atomLink_BA[0], vect_crg_M2):
                if atomM2Charge <= 1.E-6 and whichemb == 23:
                    logwrt.fatalerror('The sum of the charges of the MM atoms surrounding the boundary atom nr.' +
            ...
            if whichemb == 22:  # divide the excess charge in equal parts #USE THIS MAYBE
                ...
                logwrt.fatalerror('Invalid option for charge redistribution: ' +
            ...
            for iatom in range(1, geometry.atomNum + 1):
                if iatom in geometry.atomLink_BA[0]:
                    ...
                    logwrt.fatalerror('Incorrect atom index in charges.embeddingCharges!')
            ...
            if debug and 20 <= whichemb < 30:
                ...
                logwrt.writelog(' ka = Delta_tot / {0} = {1}\n'.format(geometry.NsubH, redistrModelCharge))
                logwrt.writelog(' kb = (charge of each M1) + ka  \n')
                logwrt.writelog(' kc = kb * sf \n')
                logwrt.writelog(' new_crg = old_crg + kc \n')
                logwrt.writelog(' diff% = (new_crg - old_crg)*100 \n\n')
        ...
        if useScaling == 3:  # do not scale
            ...
            for i in geometry.list_MM:
                ...
                if scalingFunction == '0':  # charges are zeroed
                    ...
                    logwrt.fatalerror('Incorrect scaling funct ({0}) in charges.scaledCharges!'.format(scalingFunction))
    ...
    def rallyCharges(self, CRG_model_H):
        ...
        if debug:
            ...
            logwrt.writelog('The requested scheme is nr. {0}: \n'.format(whichemb))
            if NsubH == 0:
                logwrt.writelog('There are not atom-links, so the QM charges will not be modified.\n')
            else:
                logwrt.writelog('There are {0} atom-links.\n'.format(NsubH))
                if 0 <= whichemb < 10:
                    logwrt.writelog('No QM charge modification will be performed.\n')
                    logwrt.writelog('Use this option with care: it will results in the loss of charge!\n')
                elif 10 <= whichemb < 20:
                    logwrt.writelog('Charges from previous step will be kept.\n')
                    logwrt.writelog('Use this option with care: it is intended only for debugging purposes!\n')
                elif 20 <= whichemb < 30:
                    logwrt.writelog('Charges of H atoms from modelH will be distributed among the other QM atoms.\n')
                logwrt.writelog('\n')
        ...
        if 0 <= whichemb < 10 or NsubH == 0:  # QM charges are not modified, useful for no-atom-links calculations
            ...
            if debug:
                logwrt.writelog(" Old 'model' charges sum  = {0:.6f}\n".format(self.TotCRG_model))
                logwrt.writelog(" QM  'model' charges sum  = {0:.6f}\n".format(sum(CRG_model_H_noHlink)))
                logwrt.writelog(" CRG to distribute        = {0:.6f}\n".format(CRG_to_distribute))
                logwrt.writelog(" New 'model' charges sum  = {0:.6f}\n\n".format(Tot_new_CRG_model))
                logwrt.writelog('Comparison between old charges of "model"  \n')
                logwrt.writelog('               and new charges of "model"  \n')
                logwrt.writelog('=' * 80 + '\n')
                logwrt.writelog(' (1)    (2)       (3)             (4)            (5)                (5)-(3)     \n')
                logwrt.writelog('Number Atom  model(old)      model(QM)      model(new)          Delta       %   \n')
                ...
                logwrt.writelog('=' * 80 + '\n\n')
                logwrt.writelog('  (1) Old total charge of real = {0:12f}\n'.format(self.TotCRG_model))
                logwrt.writelog('  (2) New total charge of real = {0:12f}\n'.format(Tot_new_CRG_model))
                logwrt.writelog('                       (1)-(2) = {0:12f} (should be zero)\n'.format(check1))

            if (abs(check1 - 0.0)) > 1.E-6:
                logwrt.writewarning("difference between old and new QM total charge is not zero!")
            logwrt.writelog('\n')
    ...
    def checkConsistency(self):
        ...
        logwrt.writelog('  (1) TotCRG_real            = %12f  \n' % (round(self.TotCRG_real, rnd)))
        logwrt.writelog('\n')
        logwrt.writelog('  (2) TotCRG_model           = %12f  \n' % (round(self.TotCRG_model, rnd)))
        logwrt.writelog('  (3) TotCRG_pod             = %12f  \n' % (round(self.TotCRG_pod, rnd)))
        if (abs(check1 - 0.0)) <= tlr1:
            logwrt.writelog('(ck1)       {(1)-[(2)+(3)]}  = %12f ok: should be zero \n' % (round(check1, rnd)))
            logwrt.writelog(
                '                                                it differs from zero less than %e \n' % tlr1)
        else:
            logwrt.writelog(
                '(ck1)       {(1)-[(2)+(3)]}  = %12f warning: should be zero rather than %e\n' % (check1, check1))
            logwrt.writelog(
                '                                                 it differs from zero more than %e \n' % tlr1)
        logwrt.writelog('\n')

        logwrt.writelog('  (7) TotCRG_emb             = %12f  \n' % (round(self.TotCRG_emb, rnd)))
        if (abs(check3 - 0.0)) <= tlr1:
            logwrt.writelog('(ck3)       {[(3)-(7)]}  = %12f ok: should be zero \n' % (round(check3, rnd)))
            logwrt.writelog(
                '                                                it differs from zero less than %e \n' % tlr1)
        else:
            logwrt.writelog(
                '(ck3)       {[(3)-(7)]}  = %12f warning: should be zero rather than %e\n' % (check3, check3))
            logwrt.writelog(
                '                                                 it differs from zero more than %e \n' % tlr1)
        logwrt.writelog('\n')
        logwrt.writelog('  (8) TotCRG_HIGH            = %12f  \n' % (round(self.TotCRG_HIGH, rnd)))
        logwrt.writelog('  (9) TotCRG_MEDIUM          = %12f  \n' % (round(self.TotCRG_MEDIUM, rnd)))
        logwrt.writelog(' (10) TotCRG_LOW             = %12f  \n' % (round(self.TotCRG_LOW, rnd)))
        if (abs(check4 - 0.0)) <= tlr1:
            logwrt.writelog('(ck4)  {(1)-[(8)+(9)+(10)]}  = %12f ok: should be zero \n' % (round(check4, rnd)))
            logwrt.writelog(
                '                                                it differs from zero less than %e \n' % tlr1)
        else:
            logwrt.writelog(
                '(ck4)  {(1)-[(8)+(9)+(10)]}  = %12f warning: should be zero rather than %e\n' % (check4, check4))
            logwrt.writelog(
                '                                                 it differs from zero more than %e \n' % tlr1)
        logwrt.writelog('\n')
