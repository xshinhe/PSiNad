...
class uvector {
   ...
   private:
    template <class I>
    void init_data_impl(I first, I last);
...
namespace detail {
template <class A>
inline typename std::allocator_traits<A>::pointer safe_init_allocate(
    A& alloc, typename std::allocator_traits<A>::size_type size) {
    using traits     = std::allocator_traits<A>;
    using pointer    = typename traits::pointer;
    using value_type = typename traits::value_type;
    pointer res      = alloc.allocate(size);
    if (!xtrivially_default_constructible<value_type>::value) {
        for (pointer p = res; p != res + size; ++p) { traits::construct(alloc, p, value_type()); }
    }
    return res;
}
...
template <class T, class A>
template <class I>
inline void uvector<T, A>::init_data_impl(I first, I last) {
...
inline uvector<T, A>::uvector(const uvector& rhs)
    : m_allocator(std::allocator_traits<allocator_type>::select_on_container_copy_construction(rhs.get_allocator())),
      p_begin(nullptr),
      p_end(nullptr) {
    init_data_impl(rhs.p_begin, rhs.p_end);
}
...
inline auto uvector<T, A>::at(size_type i) -> reference {
    if (i >= size()) XTENSOR_THROW(std::out_of_range, "Out of range in uvector access");
...
class svector {
   public:
    ...
    constexpr static std::size_t alignment =
        detail::allocator_alignment<A>::value != 0 ? detail::allocator_alignment<A>::value : alignof(T);
#endif
...
template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>::svector() noexcept : svector(allocator_type()) {}

template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>::svector(const allocator_type& alloc) noexcept : m_allocator(alloc) {}

template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>::svector(size_type n, const allocator_type& alloc) : m_allocator(alloc) {
    if (Init) {
        assign(n, T(0));
    } else {
        resize(n);
...
inline svector<T, N, A, Init>::svector(IT begin, IT end, const allocator_type& alloc) : m_allocator(alloc) {
    assign(begin, end);
}

template <class T, std::size_t N, class A, bool Init>
template <std::size_t N2, bool I2, class>
inline svector<T, N, A, Init>::svector(const svector<T, N2, A, I2>& rhs) : m_allocator(rhs.get_allocator()) {
...
template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>::svector(std::initializer_list<T> il, const allocator_type& alloc) : m_allocator(alloc) {
...
template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>& svector<T, N, A, Init>::operator=(svector&& rhs) noexcept(
...
inline void svector<T, N, A, Init>::assign(IT other_begin, IT other_end) {
    ...
    if (size > N && size > capacity()) { grow(size); }
...
inline auto svector<T, N, A, Init>::at(size_type idx) -> reference {
    if (idx >= size()) XTENSOR_THROW(std::out_of_range, "Out of range in svector access");
...
void svector<T, N, A, Init>::resize(size_type n) {
    ...
    size_type old_size = size();
    m_end              = m_begin + n;
    if (Init && old_size < size()) { std::fill(begin() + old_size, end(), T()); }
}
...
inline void svector<T, N, A, Init>::swap(svector<T, ON, OA, InitA>& rhs) {
    ...
    for (size_type i = 0; i < min_size; ++i) { swap((*this)[i], rhs[i]); }
...
struct array_traits {
    ...
    static constexpr T& ref(const storage_type& t, std::size_t n) noexcept { return const_cast<T&>(t[n]); }

    static constexpr T* ptr(const storage_type& t) noexcept { return const_cast<T*>(t); }
};
...
struct array_traits<T, 0> {
    ...
    static constexpr T& ref(const storage_type& /*t*/, std::size_t /*n*/) noexcept { return *static_cast<T*>(nullptr); }

    static constexpr T* ptr(const storage_type& /*t*/) noexcept { return nullptr; }
};
}  // namespace const_array_detail
#endif
...
#define XTENSOR_FIXED_SHAPE_CONSTEXPR constexpr
    ...
    constexpr static std::size_t size() { return sizeof...(X); }
    ...
    XTENSOR_FIXED_SHAPE_CONSTEXPR operator cast_type() const { return cast_type({X...}); }

    XTENSOR_FIXED_SHAPE_CONSTEXPR auto begin() const { return m_array.begin(); }

    XTENSOR_FIXED_SHAPE_CONSTEXPR auto end() const { return m_array.end(); }

    auto rbegin() const { return m_array.rbegin(); }

    auto rend() const { return m_array.rend(); }

    XTENSOR_FIXED_SHAPE_CONSTEXPR auto cbegin() const { return m_array.cbegin(); }

    XTENSOR_FIXED_SHAPE_CONSTEXPR auto cend() const { return m_array.cend(); }

    XTENSOR_FIXED_SHAPE_CONSTEXPR std::size_t operator[](std::size_t idx) const { return m_array[idx]; }

    XTENSOR_FIXED_SHAPE_CONSTEXPR bool empty() const { return sizeof...(X) == 0; }
...
sequence_view<E, Start, End>::sequence_view(const E& container) : m_sequence(container) {}

template <class E, std::ptrdiff_t Start, std::ptrdiff_t End>
template <std::ptrdiff_t OS, std::ptrdiff_t OE>
sequence_view<E, Start, End>::sequence_view(const sequence_view<E, OS, OE>& other) : m_sequence(other.storage()) {}
...
class tuple_size<xt::sequence_view<T, Start, End>> : public integral_constant<std::size_t, std::size_t(End - Start)> {};
