...
class Layers:
    ...
    def __init__(self, atomLabels: list, atomCoords: list, atomTypes: list, atomLinks: list = None):
        ...
        for i in range(len(_atom_link[0])):
            if _atom_link[0][i] in self.list_QM:
                self.atomLink_BA[1].append(_atom_link[0][i])  # A
                self.atomLink_BA[0].append(_atom_link[1][i])  # B
            elif _atom_link[0][i] in self.list_MM:
                self.atomLink_DC[1].append(_atom_link[0][i])  # C
        ...
        if self.calculationType == 'L':
            logwrt.fatalerror('No atom in the HIGH and MEDIUM levels... check your geometry definition!')
    ...
    def updatereal(self, filename="real.crd"):
        ...
        try:
            ...
            logwrt.fatalerror('{0} not found in current directory'.format(filename))
        ...
        if filename == "QMMM.in":  # read the input in the SHARC style
            ...
            if N_atoms != self.atomNum:
                logwrt.fatalerror('The number of atoms in real_layers.xyz'
    ...
    def makerealcrd(self, filename='real.crd', filename2='geometry.xyz'):
        ...
        :param filename: coordinates file name, defaults to real.crd
        """
    ...
    def displace(self, command, step):
        """
        This routine determines which atom to move, displaces it by user-specified value (default is 0.001 Angstrom)
    ...
    def read_displacement(self, step):
        """
        ...
        :return: displaced atom index and coordinates
        """
        logwrt.writelog('reading external displacement for step '+str(step)+' from external file dq'+str(step)+'.xyz\n')
        iAt = (step - 1) // 6
        iCoord = ((step - 1) % 6) // 2
        iDir = ((step - 1) % 6) % 2
        # initialize coord
