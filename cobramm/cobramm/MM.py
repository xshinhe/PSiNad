        ...
        if command[71] == '0':  # MM calculation by Amber
            ...
            if len(commandInput) == 0:
                ...
                try:
                    ...
                    freeatoms = AmberCalculator.ambermask2atomlist(self.realTop, AmberSnapshot(geometry.cartesian),
                ...
                freestring = AmberCalculator.atomlist2amberformat(freeatoms)
        ...
        # the scheme contains MM atoms, we need to do a sequence of MM calculations
            ...
            if isinstance(self.SPInput, AmberInput):  # MM calculation by Amber
                ...
                if self.doFirstStep:
                    ...
                    self.firstOutput = AmberCalculator.run(self.inputFromCMND, self.realTop, realCoords, GPU=self.GPU,
                ...
                if "H" not in geometry.calculationType:
                    ...
                    self.realOutput = AmberCalculator.run(self.SPInput, self.realTop, realCoords, GPU=self.GPU,
                                                          nCores=self.ncores, calcDir=MM._availablePathForMM())
                    logwrt.writelog(' done!\n')
                ...
                # the energy with the subtractive scheme
                    ...
                    self.realOutput = AmberCalculator.run(self.SPInput, self.realTopZeroQMCh, realCoords, GPU=self.GPU,
                                                          nCores=self.ncores, calcDir=MM._availablePathForMM())
                    logwrt.writelog(' done!\n')
                    logwrt.writelog('MM energy is {0} Hartree\n'.format(self.realOutput.energy[0]))
                    ...
                    self.modelHOutput = AmberCalculator.run(self.SPInput, self.modelHTopZeroQMCh, modelHCoords,
