...
MNDO99_ForceField::MNDO99_ForceField(const Param& iparm) : Nad_ForceField(iparm) {
    ...
    // parse mndo99 input
    std::string mndo99inp = Param_GetT(std::string, parm, "mndo99inp", "null");
    if (mndo99inp != "null") {
        natom = parse_mndo99(mndo99inp);
    } else {
        LOG(FATAL) << "Cannot find <mndo99inp>" << mndo99inp;
    }
    ...
    // FINAL REPORT
    CheckForceField();
    // LOG(INFO) << "mndo keyword" << std::endl << keyword;
}
 ...
 * @bug none
 */
int MNDO99_ForceField::ForceField_init(double* nr, double* np, double* nm, num_complex* erho, num_complex* eeac,
 ...
 * @bug none
 */
int MNDO99_ForceField::ForceField_epes(num_real* E, num_real* dE, num_real* ddE,
    ...
    std::ofstream ofs;
    ofs.open(savefile_traj, std::ios_base::app);  // save XYZ traj
    ofs << FMT(8) << natom << std::endl << FMT(8) << istep * iou.time * mod_dt << std::endl;
    for (int i = 0, idx1 = 0; i < natom; ++i) {
        ofs << FMT(8) << ELEMENTS_LABEL[atoms[i]];
        for (int k = 0; k < 3; ++k, ++idx1) ofs << FMT(8) << R[idx1] * phys::au_2_ang;
        ofs << std::endl;
    }
    ofs.close();

    ofs.open(savefile_ener, std::ios_base::app);  // save ener
    ofs << FMT(8) << istep * iou.time * mod_dt;
    for (int i = 0; i < F; ++i) ofs << FMT(8) << E[i];
    ofs << std::endl;
    ofs.close();

    ofs.open(savefile_grad, std::ios_base::app);  // save grad
    ofs << FMT(8) << istep * iou.time * mod_dt << std::endl;
    for (int j = 0; j < N; ++j) {
        for (int i = 0; i < F; ++i) ofs << FMT(8) << dE[j * FF + i * (F + 1)];
        ofs << std::endl;
    }
    ofs.close();

    ofs.open(savefile_nac, std::ios_base::app);  // save nac
    ofs << FMT(8) << istep * iou.time * mod_dt << std::endl;
    for (int j = 0; j < N; ++j) {
        for (int i = 0; i < F; ++i)
            for (int k = i + 1; k < F; ++k) ofs << FMT(8) << nac[j * FF + i * F + k];
        ofs << std::endl;
    }
    ofs.close();
 ...
 * @bug none
 */
int MNDO99_ForceField::ForceField_write(std::ofstream& ofs0, std::ofstream& ofs1, double* nr, double* np, double* nm,
                                        num_complex* erho, num_complex* eeac, int& eocc, const int& rdim,
                                        const int& fdim, const int& itraj, const int& istep) {
    // XYZ-like format output
    ofs0 << FMT(8) << natom << std::endl;                      // number of atoms
    ofs0 << FMT(8) << istep * iou.time * mod_dt << std::endl;  // time here
    for (int i = 0, idx1 = 0, idx2 = 0; i < natom; ++i) {
        // output configuration ([angstrom])
        ofs0 << FMT(8) << ELEMENTS_LABEL[atoms[i]];
        for (int k = 0; k < 3; ++k, ++idx1) ofs0 << FMT(8) << nr[idx1] * phys::au_2_ang;
        // output velocity ([angstrom/ps])
        for (int k = 0; k < 3; ++k, ++idx2) { ofs0 << FMT(8) << np[idx2] / nm[idx2] * phys::au_2_angoverps; }
        ofs0 << std::endl;
    }

    if (fdim == 1) return 0;

    // LOG(WARNING) << iou.time * mod_dt;

    // XP-Rho format output
    ofs1 << FMT(8) << F << std::endl;
    ofs1 << FMT(8) << istep * iou.time * mod_dt << FMT(8) << eocc << std::endl;
 ...
 */
int MNDO99_ForceField::parse_mndo99(const std::string& mndo99inp) {
 ...
 * @bug none
 */
std::string MNDO99_ForceField::new_keyword(const MNDO99KW_map& newkeyword) {
 ...
 * @bug none
 */
int MNDO99_ForceField::new_task(num_real* R, const std::string& file, const std::string& task_flag, const int& rdim) {
    // parse task_flag
 ...
 */
int MNDO99_ForceField::track_nac_sign() {
 ...
 * @bug none
 */
int MNDO99_ForceField::parse_standard(const std::string& log) {
    int stat = -1;
    int istate, jstate;
 ...
 * @bug none
 */
int MNDO99_ForceField::parse_hessian(const std::string& log) {
    std::ifstream ifs(log);
 ...
 * @bug none
 */
int MNDO99_ForceField::calc_normalmode(num_real* R, const int& rdim) {
 ...
 * @bug none
 */
int MNDO99_ForceField::calc_samp() {
    int stat;
...
int MNDO99_ForceField::calc_scan() {
    int istep = 0, readn;
    num_real tmp;
    std::string eachline;
    num_real* E  = new num_real[F];
