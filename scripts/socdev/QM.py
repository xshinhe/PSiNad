...
if __name__ == "__main__":
    ...
    if True:
        ...
        with open('interface.ds', 'w') as f:
            ...
            if qmmm_results.energies_triplet == None: # only singlet states
                f.write('kids_real %d\n'%len(qmmm_results.energies))
                for i in range(len(qmmm_results.energies)): # sorted order
                    f.write('{: 12.8e}\n'.format(qmmm_results.energies[i]))
                f.write('\n')
            else: # involving triplet states
                # The order of the energies is: S0, S1, S2, ..., Sn, T1(ms=-1),T1(ms=0), T1(ms=1), T2(ms=-1),T2(ms=0), T2(ms=1), ...
                f.write('kids_real %d\n'%(len(qmmm_results.energies)+3 * len(qmmm_results.energies_triplet)))
                for i in range(len(qmmm_results.energies)): # sorted order
                    f.write('{: 12.8e}\n'.format(qmmm_results.energies[i]))
            ...
            if qmmm_results.gradient_triplet == None: # only singlet states
                ...
                # The order of the gradients is: S0, S1, S2, ..., Sn, T1(ms=-1),T1(ms=0), T1(ms=1), T2(ms=-1),T2(ms=0), T2(ms=1), ...
                f.write('kids_real %d\n'%((len(qmmm_results.energies) + 3 * len(qmmm_results.energies_triplet)) * geometry.atomNum*3))
                jHM = 0 # count for H & M atoms
                # print(qmmm_results.gradient)
                for i in range(geometry.atomNum): # sorted order
                    if i+1 in geometry.list_MEDIUM_HIGH:
                        for ix in [0,1,2]:
                            for k in range(len(qmmm_results.gradient.keys())):
                                f.write('{: 12.8e} '.format(qmmm_results.gradient[k][ix][jHM]))
                            for k in range(len(qmmm_results.gradient_triplet.keys())):
                                f.write('{: 12.8e} '.format(qmmm_results.gradient_triplet[k][ix][jHM]))
                                f.write('{: 12.8e} '.format(qmmm_results.gradient_triplet[k][ix][jHM]))
                                f.write('{: 12.8e} '.format(qmmm_results.gradient_triplet[k][ix][jHM]))
                            f.write('\n')
                        jHM += 1
            ...
            if qmmm_results.energies_triplet == None: # only singlet states
                ...
                # The order of the nac is: S0-S0, S0-S1, S0-S2, ..., S0-T1(ms=-1), S0-T1(ms=0), S0-T1(ms=1), S0-T2(ms=-1),S0-T2(ms=0), S0-T2(ms=1), ..., 
                #                          S1-S0, S1-S1, S1-S2, ..., S1-T1(ms=-1), S1-T1(ms=0), S1-T1(ms=1), S1-T2(ms=-1),S1-T2(ms=0), S1-T2(ms=1), ...,
                #                          ... 
                f.write('kids_real %d\n'%((len(qmmm_results.energies) + 3 * len(qmmm_results.energies_triplet))**2*geometry.atomNum*3) )
                jHM = 0 # count for H & M atoms
                for i in range(geometry.atomNum): # sorted order
                    if i+1 in geometry.list_MEDIUM_HIGH:
                        for ix in [0,1,2]:
                            for k1 in range(len(qmmm_results.energies)):
                                for k2 in range(len(qmmm_results.energies)):
                                    if k2 == k1:
                                        ...
                                        f.write('{: 12.8e} '.format(qmmm_results.nac[k1][k2][ix][jHM]))
                                for k2 in range(3*len(qmmm_results.energies_triplet)):
                                    f.write('{: 12.8e} '.format(0)) 
                                ...
                                for ims in range(3):
                                    ...
                                    for k2 in range(len(qmmm_results.energies_triplet)):
                                        if k2 == k1:
                                            ...
                                            for ims2 in range(3):
                                                if ims2 == ims:
                                                    f.write('{: 12.8e} '.format(qmmm_results.nac_triplet[k1][k2][ix][jHM]))
                                                else:
                                                    f.write('{: 12.8e} '.format(0))
                    ...
                    if i+1 in geometry.list_LOW:
                        for ix in [0,1,2]:
                            for k1 in range(len(qmmm_results.energies)+3*len(qmmm_results.energies_triplet)):
                                for k2 in range(len(qmmm_results.energies)+3*len(qmmm_results.energies_triplet)):
                                    f.write('{: 12.8e} '.format(0))
                            f.write('\n')
            ...
            # write soc
            # The final output of the SOC is an (N_S+3N_T) x (N_S+3N_T) Hermitian matrix, where N_S is the number of singlet states and N_T is the number of triplet states.
            # The order of the row/column is: S0-S0, S0-S1, S0-S2, ..., S0-T1(ms=-1), S0-T1(ms=0), S0-T1(ms=1), S0-T2(ms=-1),S0-T2(ms=0), S0-T2(ms=1), ..., 
            #                                 S1-S0, S1-S1, S1-S2, ..., S1-T1(ms=-1), S1-T1(ms=0), S1-T1(ms=1), S1-T2(ms=-1),S1-T2(ms=0), S1-T2(ms=1), ...,
            #                                 ... 
            if qmmm_results.soc != None:
                f.write('interface.soc\n')
                f.write('kids_complex %d\n'%((len(qmmm_results.energies) + 3 * len(qmmm_results.energies_triplet))**2))
                jHM = 0 # count for H & M atoms
                for irow in range(len(qmmm_results.energies) + 3 * len(qmmm_results.energies_triplet)):
                    if irow < len(qmmm_results.energies):
                        for i in range(len(qmmm_results.energies)):
                            f.write('({:12.8e}, {:12.8e}) '.format(0,0))
                        for itriplet in range(len(qmmm_results.energies_triplet)):
                            f.write('({:12.8e}, {:12.8e}) '.format(np.real(qmmm_results.soc[irow][itriplet][0]),np.imag(qmmm_results.soc[irow][itriplet][0])))
                            f.write('({:12.8e}, {:12.8e}) '.format(np.real(qmmm_results.soc[irow][itriplet][1]),np.imag(qmmm_results.soc[irow][itriplet][1])))
                            f.write('({:12.8e}, {:12.8e}) '.format(np.real(qmmm_results.soc[irow][itriplet][2]),np.imag(qmmm_results.soc[irow][itriplet][2])))
                    else:
                        ...
                        for isinglet in range(len(qmmm_results.energies)):
                            f.write('({:12.8e}, {:12.8e}) '.format(np.real(qmmm_results.soc[isinglet][itriplet][idms]),-np.imag(qmmm_results.soc[isinglet][itriplet][idms])))
        ...
        # print('et=:', qmmm_results.energies_triplet)

        # print('gt=:', qmmm_results.gradient_triplet)

        # print('soc=:', qmmm_results.soc)
