...
class BdfOutput(QMOutput):
    ...
    def __init__(self, name, calcdir, SPcalc=False, verbose=False):
        ...
        # Change #W2 by Baihua Wu in May 10, 2025, for reading SOC
        self.dataDict["soc"] = {} # dictionary of spin-orbit coupling
        self.dataDict["energy_triplet"] = {} # dictionary of state energy of triplet states
        self.dataDict["gradient_triplet"] = {} # dictionary of state gradients of triplet states
        self.dataDict["nac_triplet"] = {} # dictionary of NAC between of triplet states
        self.dataDict["index_singlet"] = {} # index of singlet states
        # End Change #W2
        ...
        # Change #W1 by Haocheng Lu and Baihua Wu in May 10, 2025, to check BDF end normally or not
        if 'Program Stop' in ''.join(output[:]) or 'BDF STOP ...' in ''.join(output[:]) or 'STOP!' in ''.join(output[:]):
            self.dataDict["termination"] = 1
            self.dataDict["errormsg"] = ''.join(output[:])
            return
        # end Change #W1
        else:
        ...
        nstate_triplet = 0 # number of triplet states
        istate_triplet = 0 # index of triplet states
        ...
        while i < len(output):
            ...
            # find electronic energy and gradient:
            elif "GSGRAD_cstate=" in output[i] or "EXGRAD_cstate=" in output[i]:
                if 'GSGRAD_cstate' in output[i]: has_GS = True
                listfile = output[i].strip().split()
                if listfile[1] in ['0','1']: # nfile = 0: ground state; nfile = 1: singlet excited states
                    idstate = listfile[3]
                    if idstate not in self.dataDict['index_singlet'].values(): # To check if the energy/gradient of this state is already in the dictionary
                        self.dataDict['index_singlet'][istate] = idstate
                        E = float(output[i + 1].split('=')[1])
                ...
                elif listfile[1] == '2': # nfile = 2: triplet excited states
                    E = float(output[i + 1].split('=')[1])
                    self.dataDict['energy_triplet'][istate_triplet] = E
                ...
                if idfile1 != 2 and idfile2 != 2: #NAC between singlet states
                    try:
                    ...
                    try:
                        ...
                        self.dataDict["nac"][nac_jstate][nac_istate] = [[-x for x in y] for y in grad_local]
                else: #NAC between triplet states
                    try:
                        self.dataDict["nac_triplet"][nac_istate][nac_jstate] = grad_local
                    except KeyError:
                        self.dataDict["nac_triplet"][nac_istate] = {}
                        self.dataDict["nac_triplet"][nac_istate][nac_jstate] = grad_local
                    try:
                        self.dataDict["nac_triplet"][nac_jstate][nac_istate] = [[-x for x in y] for y in grad_local]
                    except KeyError:
                        self.dataDict["nac_triplet"][nac_jstate] = {}
                        self.dataDict["nac_triplet"][nac_jstate][nac_istate] = [[-x for x in y] for y in grad_local]
            ...
            # Read SOC
                ...
                soc_local = [] # ms=-1, 0, 1
                # soc_local_conj = [] # ms=-1, 0, 1

                terms = output[i+2].strip().split()
                soc_local += [float(terms[2]) + 1j * float(terms[4])]
                # soc_local_conj += [float(terms[2]) - 1j * float(terms[4])]
                terms = output[i+3].strip().split()
                soc_local += [float(terms[2]) + 1j * float(terms[4])]
                # soc_local_conj += [float(terms[2]) - 1j * float(terms[4])]
                terms = output[i+4].strip().split()
                soc_local += [float(terms[2]) + 1j * float(terms[4])]
                # soc_local_conj += [float(terms[2]) - 1j * float(terms[4])]
                ...
                try:
                    ...
                    self.dataDict["soc"][soc_state_singlet][soc_state_triplet] = soc_local
                # try:
                #     self.dataDict["soc"][soc_state_triplet][soc_state_singlet] = soc_local_conj
                # except KeyError:
                #     self.dataDict["soc"][soc_state_triplet] = {}
                #     self.dataDict["soc"][soc_state_triplet][soc_state_singlet] = soc_local_conj
        ...
        # pprint.pprint({'energy_T':self.dataDict['energy_triplet']})
        # pprint.pprint({'gradient_T':self.dataDict['gradient_triplet']})
        # pprint.pprint({'soc':self.dataDict['soc']})
        # pprint.pprint({'nac_triplet':self.dataDict['nac_triplet']})
        # pprint.pprint({'index':self.dataDict['index_singlet']})
