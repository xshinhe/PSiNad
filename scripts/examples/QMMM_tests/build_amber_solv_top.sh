...
############### CUSTOMIZE ###############
boxL=8
netcharge=1 # if the molecule has the net charge, please set with this value
# for example, Sn Calculation in MNDO with MULIT=1 need even charge. So you
# should add charge to the QM part molecule.
#########################################
...
if [ ! -f QM_ch.mol2 -o ! -f QM.frcmod ]; then
	...
	antechamber -i QM.mol2 -fi mol2 -o QM_ch.mol2 -fo mol2 -nc $netcharge -c bcc -at gaff
