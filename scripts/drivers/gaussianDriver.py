...
from psnd_env import which, source
from psnd_log import Log  # manages log file output + start/end procedures
...
class GaussianInput:
    ...
    def __init__(self, keys, gen, gaussadd, gaussweights, coords, symbols, otheropt):
        ...
        if symmetry and self.otheropt["charges"]:
            if otheropt["suppress_nosymm_error"]:
                Log.writewarning("""Allowing Gaussian calculation with symmetry... 
        Use this option with care: Gaussian expects all charges and geometries \
        in the standard orientation!!""")
            else:
                Log.fatalError("""The NoSymmetry keyword was not found in the !gaussian input section!
        ...
        if genBasis and len(gen) == 0:
            Log.fatalError("""User-specified input of the basis set is required with the GEN keyword, 
        ...
        if ischarge:
            Log.fatalError("""keyword 'charge' defined in the cobram.command. Please, remove it.
    ...
    def checkEnv():
        ...
        try:
            version = os.environ["GAUSSIAN_EXE"]
        ...
        # check that the version that is passed corresponds to a supported version
        if version != "g03" and version != "g09" and version != "g16":
            return False, "{0} version of Gaussian is not currently supported by Cobramm".format(version)

        # source the configuration file for this version of gaussian
        try:
            setGaussianProfile(version, gpath)
        except:
            return False, "error encountered while sourcing the {0} profile script".format(version)

        # check if gaussian executable is available
        if not which(version):
            message = version + " executable is not available"
    ...
    def fileText(self, inpFileName, memory="500MB", nproc=1, version="g16"):
        ...
        the number of cores (var nproc) to use in the execution and the version of Gaussian (version)
        ...
        if self.otheropt["tdcouplings"]:
            if _optionInRoute(self.keys, ['td']):  # when the TD option is active, substitute it with TDA (Tamm-Damkov)
                Log.writewarning("Calculation of time derivative couplings is only possible with TDA...replacing TD keyword in Gaussian input\n")
        ...
        if self.otheropt["nstate"] is not None:
            if _optionInRoute(self.keys, ['td', 'tda']):  # when the calculation is TD or TDA
                if self.otheropt["nstate"] == 0:
                    ...
                    if findroot:
                        if self.otheropt["nstate"] != int(findroot[0].split("=")[1]) and \
                            ...
                            Log.fatalError("Mismatch of the electronic state between command file and QM input")
                ...
                Log.fatalError("cannot change the electronic state number for these Gaussian options")
        ...
        if version == 'g16' and len(self.gaussweights) != 0:
        ...
        if version != 'g16' and len(self.gaussadd) != 0:
            for line in self.gaussweights:
                inputText += line + '\n'
            inputText += '\n'
...
class GaussianOutput(QMOutput):
    ...
    def __init__(self, name, calcdir, SPcalc=False, verbose=False):
        ...
        if SPcalc == False and verbose == False:
            self.dataDict["outfile"] = self._filterOutput(os.path.join(calcdir, name))
        else:
    ...
    def __del__(self):
        ...
        #            source=self.dataDict["calcdir"]+"/gaussian-QM.log"
    ...
    def orbfiles(self):
        ...
        chkname = os.path.join(self.dataDict["calcdir"], self.dataDict["inpfile"]+".chk")

        # if the file is not there, print warning and return an empty list
        if not os.path.exists(chkname):
            Log.writewarning("gaussian file {0} cannot be found: it will not be stored".format(chkname))
            return []
