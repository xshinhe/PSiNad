...
from QMOutput import QMOutput, QMOutput0
...
class MNDOInput:
    ...
    def __init__(self, keys, coords, symbols, otheropt):
        """Constructor of the mndoInput class, requires these arguments:
        - keys: string with the text of the mndo inp file from the route section to the charge/mult line
        ...
        -
        """
        # store the sections of the mndo input files already defined
        self.keys = keys
        self.coords = coords
    ...
    def checkEnv():
        ...
        return True, mndopath


    def fileText(self, inpFileName, memory="500MB", nproc=1, version="mndo2020"):
        """Prepare the text of mndo input file named after the inpFileName variable (input file
        should be named <inpFileName>.com, so inpFileName should not include the extension!), using the
        input data defined in the instance of mndoInput. The memory (var memory) and
...
class MNDOOutput(QMOutput):
    ...
    def __init__(self, name, calcdir, calctype, SPcalc=False, verbose=False):
        ...
        with open(os.path.join(calcdir, name)) as fout:
            self.dataDict["outfile"] = fout.read()
            self.QMDict.outfile = self.dataDict['outfile']

            # print(self.dataDict["outfile"])

        # print(type(self.QMDict.outfile))
        # pprint(self.QMDict)
    ...
    def __del__(self):
        """Destructor for the mndoOutput class: not only the memory allocated for the oject attributes
        ...
        # try:
            # if not kids_log.DEBUG_RUN:
                # if self.dataDict["termination"] == 1:
                    # source=self.dataDict["calcdir"]+"/mndo-QM.log"
...
if __name__ == "__main__":
    ...
    file = 'mndo-QM.log'
    calcdir = 'qm_mndo/qmCalc00001'
    OUT = MNDOOutput(file, calcdir, 'OM2/MRCI')
