...
type_list = ['void', 'bool', 'int', 'double', 'psnd_real', 'psnd_complex',
...
def creat_class_var(ns, dd):
    for f in dd[ns]['_var']:
        for i in f:
            if f[i]['_attr'] == 'public':
                if f[i]['_type'] in ['int', 'double', 'psnd_real', 'psnd_complex',
...
def creat_class_fun(ns, ns0, dd):
    for f in dd[ns]['_fun']:
        for i in f:
            if f[i]['_type'] not in type_list or 'psnd_complex*' in f[i]['_raw']:
                continue
