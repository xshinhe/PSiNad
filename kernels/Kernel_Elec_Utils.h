...
static int c_binning(kids_complex* ker, kids_complex* rho, int sqc_type) {
...
int max_choose(kids_complex* rho) {
    int       imax = 0;
    kids_real vmax = 0.0f;
...
int pop_choose(kids_complex* rho) {
    kids_real rand_tmp;
    kids_real sum = 0.0f;
...
int pop_neg_choose(kids_complex* rho) {
    kids_real rand_tmp;
    kids_real sum = 0.0f;
...
int hopping_choose(kids_complex* rho, kids_complex* H, int from, kids_real dt) {
    int       to = from;
...
void hopping_direction(kids_real* direction, kids_real* dE, int from, int to) {
    if (to == from) return;
    for (int i = 0; i < Dimension::N; ++i) { direction[i] = dE[i * Dimension::FF + from * Dimension::F + to]; }
}

int hopping_impulse(kids_real* direction, kids_real* np, kids_real* nm, kids_real* E,  //
