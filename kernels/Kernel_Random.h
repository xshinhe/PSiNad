...
class Kernel_Random : public Kernel {
   public:
    ...
    static inline int rand_uniform(kids_real* res_arr, int N = 1, kids_real sigma = 1.0) {
        for (int i = 0; i < N; ++i) { res_arr[i] = sigma * rand_udd(rand_rng); }
        return 0;
    }

    static inline int rand_gaussian(kids_real* res_arr, int N = 1, kids_real sigma = 1.0,
                                    kids_real mu = 0.0) {  // parse mu after sigma !!!
        for (int i = 0; i < N; ++i) { res_arr[i] = mu + sigma * rand_nd(rand_rng); }
        return 0;
    }

    static inline int rand_exponent(kids_real* res_arr, int N = 1) {
        kids_real randu;
        for (int i = 0; i < N; ++i) {
            rand_uniform(&randu);
    ...
    static inline int rand_poisson(int* res_arr, int N = 1, kids_real lambda = 1.0f) {
    ...
    static inline int rand_simplex(kids_real* res_arr, int N = 1, kids_real constr = 1.0f) {
    ...
    static inline int rand_sphere(kids_real* res_arr, int N = 1, kids_real constr = 1.0f) {
        kids_real norm = 0.0f;
        for (int i = 0; i < N; ++i) {
