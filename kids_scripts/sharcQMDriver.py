...
class SharcQMInput:
    ...
    def __init__(self, qmcode, coords, symbols, otheropt):
        """Constructor of the SharcQMInput class, requires these arguments:
        ...
        # initialize attributes of the SharcQMInput class instance
    ...
    def chargeText(self):
        ...
        if self.qmcode.upper() == "MOLCAS":
           ...
           Log.fatalError('Cannot write charge.dat for SHARC_{0}.py SHARC interface'.format(self.qmcode))
...
class SharcQMOutput(QMOutput):
    ...
    def __init__(self, name, calcdir):
        ...
        Log.writeLog("\n", 1)
        Log.writeLog("Reading output files from SHARC interface\n", 1)
        Log.writeLog("Currently working in directory {0}\n".format(os.getcwd()), 1)
        Log.writeLog("Reading files from directory {0}\n".format(calcdir), 1)
        Log.writeLog("Content of the calculation dir:\n{0}\n".format(os.listdir(calcdir)), 1)
        Log.writeLog("\n", 1)
        ...
        try:
            ...
            Log.fatalError('Cannot find the output file from SHARC QM interface {0}...\n'
    ...
    def __del__(self):
        ...
        if not kids_log.DEBUG_COBRAMM_RUN: os.remove(os.path.join(self.dataDict["dir"], self.dataDict["name"]+".out"))
        if not kids_log.DEBUG_COBRAMM_RUN: os.remove("charge.dat")
        # clean up the log
        del self.log
