...
import softenv  # function to check if executables are available
...
class PumpProbeDriver:
    ...
    def check_folder(folder):
        ...
        termination_files = ["FINISHED", "old.xml"]
        ...
        with open("setup_PP.log", "w") as out:
            ...
            for step in range(0, self.nsteps+1, self.delta):
                ...
                if self.method == 0:
                    ...
                    PumpProbeDriver.launch_cobramm(cobcomm, submission_string, cluster)
                if self.method == 1:
                    ...
                    PumpProbeDriver.launch_cobramm(cobcomm_first, submission_string)
                    os.chdir(upper)
                    shutil.copyfile("{}/vanilla.JobMix".format(os.path.abspath(reference_dir)), "vanilla.JobMix")
                    PumpProbeDriver.launch_cobramm(cobcomm_second, submission_string, cluster)
    ...
    def get_dipoles(method, calcDir):
        ...
        if method == 0:
            # run MultiWfn
            dipoles = MultiwfnDriver.extract_TDM_MWFN(calcDir, "restart.chk")
        if method == 1:
            onlydipoles, nstates = PumpProbeDriver.extract_tdm_molcas(calcDir)
            cas_energies = PumpProbeDriver.extract_pt2_energies_molcas(calcDir)
            dipoles = PumpProbeDriver.update_dipoles_cas(nstates, energies=cas_energies, dipoles=onlydipoles)
    ...
    def update_dipoles_cas(nstates, energies: list, dipoles: list):
        ...
        for gs_to_es in range(nstates-1):
            ...
            mag = PumpProbeDriver.calculate_tdm_magnitude(dipoles[gs_to_es][2], dipoles[gs_to_es][3], dipoles[gs_to_es][4])
            fos = PumpProbeDriver.calculate_fos(mag, dipoles[gs_to_es][-1])
        ...
        for es_to_es in range(nstates-1,len(dipoles)):
            ...
            mag = PumpProbeDriver.calculate_tdm_magnitude(dipoles[es_to_es][2], dipoles[es_to_es][3], dipoles[es_to_es][4])
            fos = PumpProbeDriver.calculate_fos(mag, dipoles[es_to_es][-1])
    ...
    #        dipoles = MultiwfnDriver.extract_TDM_MWFN(calcDir, "restart.chk")
    #    if self.method == 1:
    #        onlydipoles = PumpProbeDriver.extract_tdm_molcas()
    #        cas_energies = PumpProbeDriver.extract_energies_molcas(nstates, calcDir)
    #        dipoles = PumpProbeDriver.update_dipoles_cas(nstates, energies=cas_energies, dipoles=onlydipoles, fos=fos)
    ...
    def extract_init_tdm(self):
        ...
        if self.method == 0:
            ...
            for folder in self.traj:
                os.chdir(folder)
                active_state = PumpProbeDriver.get_active_states(0)
                os.chdir("inputs_from_step_0")
                try:
                    dipoles = MultiwfnDriver.extract_TDM_MWFN(".", "restart.chk")
    ...
    def extract_en_fos_polarization(method, calcDir, nstates: int, active_state: int, polarization, init_tdm=[]):
        ...
        #    dipoles = MultiwfnDriver.extract_TDM_MWFN(calcDir, "restart.chk")
        #if self.method == 1:
        #    onlydipoles = PumpProbeDriver.extract_tdm_molcas(nstates, calcDir)
        #    cas_energies = PumpProbeDriver.extract_energies_molcas(nstates, calcDir)
        #    dipoles = PumpProbeDriver.update_dipoles_cas(nstates, energies=cas_energies, dipoles=onlydipoles)

        dipoles = PumpProbeDriver.get_dipoles(method, calcDir=calcDir)
    ...
    def decompose_esa(calcDir, nstates:int, active_state: int, target_state: int):
        ...
        if self.method == 0:
            ...
            dipoles = MultiwfnDriver.extract_TDM_MWFN(calcDir, "restart.chk")
        if self.method == 1:
            onlydipoles = PumpProbeDriver.extract_tdm_molcas(calcDir)
            cas_energies = PumpProbeDriver.extract_pt2_energies_molcas(calcDir)
            dipoles = PumpProbeDriver.update_dipoles_cas(nstates, energies=cas_energies, dipoles=onlydipoles)
    ...
    def collect_values_single_time(self, nstep:int, polarization="none", all_init_tdm=[]):
        ...
        for folder in self.traj:
            ...
            active_state = PumpProbeDriver.get_active_states(nstep)
            ...
            if self.method == 1:
                ovlp = PumpProbeDriver.extract_overlap_molcas(self.nstates)
                state_ovl = ovlp[active_state]
                new_active_state = state_ovl.index(max(state_ovl))


            en, fos = PumpProbeDriver.extract_en_fos_polarization(self.method, ".", self.nstates, new_active_state,
    ...
    def collect_decomposed_values_single_time(self, nstep:int,selected_state,target_state):
        ...
        for folder in self.traj:
            ...
            if active_state == selected_state:
                os.chdir("inputs_from_step_{}".format(nstep))
                try: ##### to speed up tests
                    en, fos = PumpProbeDriver.decompose_esa(".", self.nstates, active_state, target_state)
                    for state in range(len(en)):
                        collected_energies.append(en[state])
                        collected_intensities.append(fos[state])
                except:
...
#####################################################################################################
class SpectronDriver:
    spectron_dir = ""
    multiwfn_dir = ""
    _SpectronCheck = False

    def __init__(self):
        """ the constructor of the SpectronDriver class checks if the environment for SPECTRON execution
            is properly defined and if all the necessary executables are available """

        if not SpectronDriver._SpectronCheck:

            # environmental variable with the COBRAMM path
            try:
                SpectronDriver.spectron_dir = os.environ['SPECTRON_DIR']
            except KeyError:
    ...
    def run_pp(inpDir, outDir="2D_data", inputfile="input.com", simulation_time=int, time_step=int):
        ...
        for step in range(0, simulation_time + 1, time_step):
            ...
            SpectronDriver.runSpectron(calcDir=".")
...
class MultiwfnDriver:
    ...
    def __init__(self):
        ...
        if not MultiwfnDriver._multiwfnCheck:

        # check multiwfn and setup
            try:
                MultiwfnDriver.multiwfn_dir = os.environ['Multiwfnpath']
            except KeyError:
                raise MultiwfnError("environment variable $Multiwfnpath is not defined")

            if not softenv.which("Multiwfn"):
