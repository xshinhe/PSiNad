...
class AdfInput:
    """Store the data for the execution of Adf and process it to check its consistency"""

    def __init__(self, keys, coords, symbols, otheropt):
        """Constructor of the adfInput class, requires these arguments:
        - keys: string with the text of the adf inp file from the route section to the charge/mult line
        ...
        -
        """
        # store the sections of the adf input files already defined
        self.keys = keys
        self.coords = coords
    ...
    def fileText(self, inpFileName, memory="500MB", nproc=1, version="adf2020"):
        """Prepare the text of adf input file named after the inpFileName variable (input file
        should be named <inpFileName>.com, so inpFileName should not include the extension!), using the
        input data defined in the instance of adfInput. The memory (var memory) and
        the number of cores (var nproc) to use in the execution and the version of Adf (gversion)
         are given as input because they are decided when running the Adf calculation and are not
...
class AdfOutput(QMOutput):
    ...
    def __init__(self, name, calcdir, calctype, SPcalc=False, verbose=False):
        ...
        self.dataDict["outfile"] = None   # string with the text of the Adf log file
        ...
        # store the adf log removing AO and MO definitons, CI expansions, etc.
        ...
        while i < len(output):
            ...
            # get gradient or nac (including MM points)
                ...
                for _ in range(10):
                    if 'EXTERNAL POINT CHARGES' in output[k]:
                        # note: adf doesn't count the interaction of external charged (only H+H & H+L)
            ...
            # get electrostatic field at MM point charges (need mminp = 1, or revised adf-code with mmcoup>=3)
                ...
                while output[k].strip() != '':
                    ...
                    k += 1
                # Adf only provide elfield for ground state, here we may copy it to all states
    ...
    def __del__(self):
        """Destructor for the adfOutput class: not only the memory allocated for the oject attributes
        (the dictionary self.dataDict) needs to be released, also the Adf I/O files stored on disk
        can be safely removed... """

        # permanently remove directory and adf files
        # try:
            # if not kids_log.DEBUG_COBRAMM_RUN:
                # if self.dataDict["termination"] == 1:
                    # source=self.dataDict["dir"]+"/adf-QM.log"
...
if __name__ == "__main__":
    # check adfDriver's parser 

    name = sys.argv[1]
    test = AdfOutput(name, '.', 'OM2-MRCI')
