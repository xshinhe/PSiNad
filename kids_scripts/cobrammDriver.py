...
import softenv  # general purpose functions in use in COBRAMM
import constants  # value of physical constants of common use

# imports of local objects

from amberDriver import AmberDriver, AmberSnapshot, AmberTopology  # classes that constitute the Amber wrapper
from Layers import Layers  # class to define the geometry of a QM/MM calculation
from Output import Output  # class to parse and write the old.xml file
...
class CobrammDriver:
    ...
    def __init__(self):
        ...
        if not CobrammDriver._cobrammCheck:
    ...
    def run(inputfile, geometry, modeltopology, realtopology, calc_dir="cobrammCalc", n_cores=1, store=False):
        ...
        with open(os.path.join(calc_dir, CobrammDriver.MODELHTOP), "w") as f:
            f.write(modeltopology.topotext)
        with open(os.path.join(calc_dir, CobrammDriver.REALTOP), "w") as f:
            f.write(realtopology.topotext)
        # write geometry and layers to the real layers file
        with open(os.path.join(calc_dir, CobrammDriver.REALLAYERS), "w") as f:
            f.write(geometry.reallayertext)
        # write cobramm command file
        with open(os.path.join(calc_dir, CobrammDriver.COBRAMMCOMMAND), "w") as f:
            f.write(inputfile.inputtext)

        # run the COBRAMM calculation
        os.chdir(calc_dir)
        with open(CobrammDriver.COBRAMMLOG, "w") as fstdout:
        ...
        for j, iAtom2 in enumerate(geometry.list_MEDIUM):
            for iAtom1 in geometry.list_MEDIUM[0:j]:
                ...
                if lab1 == "H" or lab2 == "H":
                    ...
                    if actDistance < refDistance * CobrammDriver._BOND_TOLERANCE:
    ...
    def qmmmLayers(realSnapshot: AmberSnapshot, realTopology: AmberTopology, nrResidue: int = 1,
        ...
        symbolList, coordsMatrix, residueList = AmberDriver.extractAtomsAndResidues(realTopology, realSnapshot)
        ...
        for iRes in range(max(residueList)):
            ...
            # compute distance
            distance = CobrammDriver.residueDistance(QMResidue, currentMMResidue, "min_min")
...
class CobrammOutput:
    ...
    def __init__(self, out_dir, store_files=False):
        ...
        if self.xmloutput.normal_modes:
            ...
            softenv.setKIDSProfile()
            # check that the environment for gaussian is actually defined
            env_defined, error_msg = softenv.checkGaussianOptEnv()
            if not env_defined:
...
if __name__ == '__main__':
    ...
    with open(os.path.join(testdir, CobrammDriver.MODELHTOP)) as ff:
        testmodelhtop = AmberTopology(ff.read())
    with open(os.path.join(testdir, CobrammDriver.REALTOP)) as ff:
        testrealtop = AmberTopology(ff.read())
    with open(os.path.join(testdir, CobrammDriver.REALLAYERS)) as ff:
    ...
    optresult = CobrammDriver.run(cobcomm, testgeometry, testmodelhtop, testrealtop,
    ...
    # run the cobramm optimization and extract the optimized geometry
    tddftresult = CobrammDriver.run(cobcomm, optgeometry, testmodelhtop, testrealtop,
