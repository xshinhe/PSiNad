...
class QchemInput:
    """Store the data for the execution of Qchem and process it to check its consistency"""
    ...
    def fileText(self, inpFileName, memory="500MB", nproc=1, version="bdf2020"):
        ...
        the number of cores (var nproc) to use in the execution and the version of Qchem (gversion)
         are given as input because they are decided when running the Qchem calculation and are not
...
class QchemOutput(QMOutput):
    ...
    def __init__(self, name, calcdir, calctype, SPcalc=False, verbose=False):
        ...
        self.dataDict["outfile"] = None   # string with the text of the Qchem log file
        ...
        while i < len(output):
            ...
            # get electrostatic field at MM point charges (need mminp = 1, or revised bdf-code with mmcoup>=3)
                ...
                while output[k].strip() != '':
                    ...
                    k += 1
                # Qchem only provide elfield for ground state, here we may copy it to all states
    ...
    def __del__(self):
        ...
        (the dictionary self.dataDict) needs to be released, also the Qchem I/O files stored on disk
...
if __name__ == "__main__":
    ...
    name = sys.argv[1]
    test = QchemOutput(name, '.', 'OM2-MRCI')
