...
if __name__ == "__main__":
    ...
    confFile = softenv.setKIDSProfile()
    ...
    totalTimer.start()
    if os.path.exists('old.xml'): os.remove('old.xml')
    # logwrt.cobramstart()
    ...
    logwrt.writeLog(softenv.checkKIDSProfile(confFile))
    ...
    #CRG_real = None # amber.prepare(geometry, cobcom, command)
    CRG_real = amber.read_crgA()
    ...
    try:
        ...
        real_top = None
    # init Output instance and write first part of the old.xml file
    ...
    while True:
        logwrt.startSection('QM SINGLE POINT')
        ...
        with open('interface.ds', 'w') as f:
            ...
            f.write('kids_real %d\n'%len(QMCalc.energydict))
            for i in range(len(QMCalc.energydict)): # sorted order
                f.write('{: 12.8e}\n'.format(QMCalc.energydict[i]))
            f.write('\n')

            # write energy
            f.write('interface.dE\n')
            f.write('kids_real %d\n'%(len(QMCalc.energydict) * geometry.atomNum*3))
            jHM = 0 # count for H & M atoms
            for i in range(geometry.atomNum): # sorted order
                if i+1 in geometry.list_MEDIUM_HIGH:
                    for ix in [0,1,2]:
                        for k in range(len(QMCalc.energydict)):
                            f.write('{: 12.8e} '.format(QMCalc.gradientdict[k][ix][jHM]))
                        f.write('\n')
                    jHM += 1
            ...
            f.write('kids_real %d\n'%(len(QMCalc.energydict)*len(QMCalc.energydict)*geometry.atomNum*3) )
            jHM = 0 # count for H & M atoms
            for i in range(geometry.atomNum): # sorted order
                if i+1 in geometry.list_MEDIUM_HIGH:
                    for ix in [0,1,2]:
                        for k1 in range(len(QMCalc.energydict)):
                            for k2 in range(len(QMCalc.energydict)):
                                if k2 == k1:
                                    f.write('{: 12.8e} '.format(0))
                                else:
                                    f.write('{: 12.8e} '.format(QMCalc.nacdict[k1][k2][ix][jHM]))
                        f.write('\n')
                    jHM += 1
                if i+1 in geometry.list_LOW:
                    for ix in [0,1,2]:
        ...
        logwrt.printEnergies(QMCalc.energy(), E_modelH, E_modelnoc, E_second, QMCalc.selfenergy,
                             QMCalc.getenergy(), geometry.calculationType)
