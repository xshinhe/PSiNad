...
class PyscfInput:
    """Store the data for the execution of Pyscf and process it to check its consistency"""

    def __init__(self, keys, coords, symbols, otheropt):
        """Constructor of the pyscfInput class, requires these arguments:
        - keys: string with the text of the pyscf inp file from the route section to the charge/mult line
        ...
        -
        """
        # store the sections of the pyscf input files already defined
        self.keys = keys
        self.coords = coords
    ...
    @staticmethod
    def checkEnv():
        """ the function checks if the environment for Pyscf execution
        ...
        try:
            pyscfversion = os.environ["ORCA_EXE_QM"]
            pyscfpath = os.environ["ORCA_DIR_QM"]
        except KeyError:
            return False, "environment variables for Pyscf, $ORCA_EXE_QM, " \
                          "$ORCA_DIR_QM are not defined"

        # check that the gversion that is passed corresponds to a supported version
        #if pyscfversion != "pyscf99" and pyscfversion != "pyscf2020":
        #    return False, "{0} version of pyscf is not currently supported by Cobramm".format(pyscfversion)

        # check if pyscf executable is available
        if not which(pyscfversion):
            message = pyscfversion + " executable is not available"
            return False, message

        # return True if all the checks were OK
        return True, pyscfpath


    def fileText(self, inpFileName, memory="500MB", nproc=1, version="pyscf"):
        """Prepare the text of pyscf input file named after the inpFileName variable (input file
        should be named <inpFileName>.com, so inpFileName should not include the extension!), using the
        input data defined in the instance of pyscfInput. The memory (var memory) and
        the number of cores (var nproc) to use in the execution and the version of Pyscf (gversion)
         are given as input because they are decided when running the Pyscf calculation and are not
...
class PyscfOutput(QMOutput):
    ...
    def __init__(self, name, calcdir, calctype, SPcalc=False, verbose=False):
        ...
        self.dataDict["outfile"] = None   # string with the text of the Pyscf log file
        ...
        # store the pyscf log removing AO and MO definitons, CI expansions, etc.
        ...
        while i < len(output):
            ...
            # get gradient or nac (including MM points)
                ...
                for _ in range(10):
                    if 'EXTERNAL POINT CHARGES' in output[k]:
                        # note: pyscf doesn't count the interaction of external charged (only H+H & H+L)
            ...
            # get electrostatic field at MM point charges (need mminp = 1, or revised pyscf-code with mmcoup>=3)
                ...
                while output[k].strip() != '':
                    ...
                    k += 1
                # Pyscf only provide elfield for ground state, here we may copy it to all states
    ...
    def __del__(self):
        """Destructor for the pyscfOutput class: not only the memory allocated for the oject attributes
        (the dictionary self.dataDict) needs to be released, also the Pyscf I/O files stored on disk
        can be safely removed... """

        # permanently remove directory and pyscf files
        # try:
            # if not kids_log.DEBUG_RUN:
                # if self.dataDict["termination"] == 1:
                    # source=self.dataDict["calcdir"]+"/pyscf-QM.log"
                    # shutil.move(source,"pyscf-QM_err.log")
...
if __name__ == "__main__":
    ...
    file = 'pyscf-QM.log'
    calcdir = 'qm_pyscf/qmCalc00001'
    OUT = PyscfOutput(file, calcdir, 'OM2/MRCI')
