...
def prepare(geometry, ks_config):
    ...
    envDefined, errorMsg = softenv.checkAmberEnv()
    ...
    cretesandersecond(low_sander_input,geometry,ks_config)
    return CRG_real
...
def MMcalculations(geometry, ks_config):
    ...
    if "L" in geometry.calculationType and ifskip == 0:  # low optimization is not skipped
        sanderfirst(ks_config) # optimization of low
    ...
    E_second, Fxyz_second = sandersecond(geometry, ks_config)
    #####################
    # amber modelnoc MM single point on real w ZERO charges on model-H
    E_modelnoc, Fxyz_modelnoc = sandermodelnoc(ks_config)
    #####################
    ...
    E_modelH, Fxyz_modelH = sandermodelH(geometry,ks_config)
...
def cretesandersecond(low_sander_input,geometry,ks_config):
...
# (OW) Do the pmemd trick, compute one MD step in dielektrikum to obtain velocities for GPU-code
        if ks_config.get('MM.AMBER.type', 'serial')=='gpu':
            low_sander_input= ["minimizzazione","&cntrl","imin = 1,","ncyc = 1,","maxcyc = 0,","ntb = 0,","igb = 1,","extdiel = 1","ntr = 0,","DRMS = 0.0005,","ntxo=1,","cut = "+ks_config.get('MM.AMBER.cutoff', '999'),"/"]
        else:
            low_sander_input= ["minimizzazione","&cntrl","imin = 1,","ncyc = 1,","maxcyc = 0,","ntb = 0,","igb = 0,","ntr = 0,","DRMS = 0.0005,","ntxo=1,","cut = "+ks_config.get('MM.AMBER.cutoff', '999'),"/"]
...
# do the pmemd-trick to get forces from velocities (OW)
        sander.write('  cut = '+ ks_config.get('MM.AMBER.cutoff', '999')+',\n')
...
def sandersecond(geometry,ks_config):
...
def sandermodelH(geometry,ks_config):
